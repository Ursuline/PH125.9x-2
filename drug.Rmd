...
None of the modeling approaches used provided an improvement over linear regression (`r sprintf("%0.1f%%",(CM.glm$overall)[1]*100)` accuracy). With linear regression, Country of origin, Age, Openness to experiment and Education are the factors contributing most to the accuracy. 
...
Linear modeling, offers a (`r sprintf("%0.1f%%",((CM.glm$overall)[1]-naive)*100)`) improvement over the naive approach and the neural network (`r sprintf("%0.1f%%",((CM.nnet$overall)[1]-naive)*100)`).
...
We felt this may be due to the choice of classification and that a potentially even more insightful analysis would take into account frequency of use (this information was nevertheless not available in the dataset). 
